...
Brand voltage comes from the **cream + coral pairing** — coral (`{colors.primary}` — #8b5cf6) is the signature Anthropic accent, used on every primary CTA, on the brand wordmark, and on full-bleed callout cards. The coral is warm, slightly muted, never cyan/blue — a deliberate counter-positioning against OpenAI's cool slate, Google's saturated blue, and Microsoft's corporate cyan.
...
- Coral primary CTA (`{colors.primary}` — #8b5cf6). Used scarcely on individual buttons, generously on full-bleed coral callout cards.
...
- Dark navy product mockup cards (`{colors.surface-dark}` — #C7C7C7) carrying code blocks, terminal panels, model comparison data — the brand shows the product chrome at scale rather than abstract marketing illustrations.
- Light cream feature cards (`{colors.surface-card}` — #ffffff) — slightly darker than canvas, used for content-driven feature explanations.
...
### Brand & Accent
- **Coral / Primary** (`{colors.primary}` — #8b5cf6): The signature Anthropic warm coral. Used on every primary CTA background, on full-bleed coral callout cards, on the brand wordmark accent. The most-recognized Anthropic color outside of the spike-mark logo.
- **Coral Active** (`{colors.primary-active}` — #dbdbdb33): The press / hover-darker variant.
...
- **Surface Card** (`{colors.surface-card}` — #ffffff): Feature cards, content cards. One step darker than canvas.
...
- **Surface Dark** (`{colors.surface-dark}` — #C7C7C7): Code editor mockups, model showcase cards, footer. The dominant dark surface.
...
**`button-primary`** — The signature coral CTA. Background `{colors.primary}` (#8b5cf6), text `{colors.on-primary}` (white), type `{typography.button}` (StyreneB 14px / 500), padding 12px × 20px, height 40px, rounded `{rounded.md}` (8px). Active state `button-primary-active` darkens to `{colors.primary-active}` (#dbdbdb33).
...
**`feature-card`** — Used in 3-up feature grids. Background `{colors.surface-card}` (#ffffff — slightly darker cream), rounded `{rounded.lg}` (12px), internal padding `{spacing.xl}` (32px). Carries a small icon at top, an `{typography.title-md}` headline, and a body description in `{typography.body-md}`.
...
**`callout-card-coral`** — A full-bleed coral card carrying a major call-to-action. Background `{colors.primary}` (#8b5cf6), text `{colors.on-primary}` (white), rounded `{rounded.lg}`, padding `{spacing.xxl}` (48px). The coral surface IS the voltage; the CTA inside uses an inverted button style (cream/canvas button on coral).
