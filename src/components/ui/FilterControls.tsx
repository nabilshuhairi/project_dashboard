...
function DropdownSelect<T extends string>({ label, value, options, onChange }: DropdownSelectProps<T>) {
  ...
  return (
    <div className="relative flex h-full flex-col justify-between" ref={menuRef}>
      ...
      <button
        type="button"
        className="flex h-14 w-full items-center justify-between rounded-2xl border border-[#252320] bg-slate-100 px-4 text-left text-sm text-black shadow-sm transition focus:border-[#dbdbdb33] focus:outline-none focus:ring-2 focus:ring-[#dbdbdb33]/20"
      ...
      {isOpen && (
        <div className="absolute left-0 right-0 top-full z-10 mt-1 max-h-56 overflow-auto rounded-2xl border border-[#252320] bg-slate-100 shadow-lg">
          {options.map((option) => (
            <button
              key={option.value}
              type="button"
              className="w-full px-4 py-3 text-left text-sm text-black transition hover:bg-slate-200"
              onClick={() => {
    ...
    <div className="grid gap-4 rounded-xl border border-hairline bg-surface-card p-6 shadow-lg shadow-slate-900/5 dark:border-surface-dark-elevated dark:bg-surface-dark sm:grid-cols-2 xl:grid-cols-3">
      ...
      <div className="flex h-full flex-col justify-between">
        ...
        <input
          ...
          className="h-14 w-full rounded-2xl border border-[#252320] bg-slate-100 px-4 text-sm text-black placeholder:text-slate-500 shadow-sm outline-none transition focus:border-[#dbdbdb33] focus:ring-2 focus:ring-[#dbdbdb33]/20"
        />
      </div>
